...
async function fetch_weather_data() {
  try {
    ...
    const points_res = await fetch(
      `https://api.weather.gov/points/${weather_lat},${weather_lon}`
    );
    ...
    if (!daily_forecast_res.ok || !hourly_forecast_res.ok) throw new Error('Failed to fetch forecasts');
